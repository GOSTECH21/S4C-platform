...
export default async function GoalAlertPage({
  searchParams,
}: PageProps) {
  const params = await searchParams;

  const sport = params.sport ?? "Football";
  const team = params.team ?? "Arsenal";
  const score = params.score ?? "Arsenal 1 - 0 Liverpool";
    ...
    <main className="flex min-h-screen items-center justify-center bg-slate-950 p-8 text-white">
      <div className="w-full max-w-2xl rounded-3xl border border-green-400 bg-slate-900 p-10 text-center shadow-2xl">
        ...
        <h2 className="mt-8 text-2xl font-bold">
          {sponsors.length} sponsors have released Climate Credits
        </h2>
